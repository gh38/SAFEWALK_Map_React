...
function initMap() {
  ...
  getCurrentLocation();
  window.ReactNativeWebView.postMessage(JSON.stringify({ key1: "NOTIFICATION", key2: `2` }));
...
const Map = ({ children }) => {
  ...
  const [activeInput, setActiveInput] = useState(null); // 현재 활성화된 입력란을 추적하기 위한 상태
    ...
    if (data.response.status === "NOT_FOUND") {
      ...
      for (var i = 0; i < data.response.result.items.length; i++) {
        ...
        if (i === 0) {
          map
            .getView()
            .setCenter([datalists.point.x * 1, datalists.point.y * 1]);
          map.getView().setZoom(17);
        }
        ...
        var marker = new Feature({
          ...
          content: datalists.address.road
            ? datalists.address.road
            : datalists.address.parcel, // 도로명 주소가 있으면 road, 없으면 parcel
        });
  ...
  useEffect(() => {
    ...
    if (selectedGrade > 0 && selectedGrade !== "0") {
      // 확인 후 변경 (값을 선택 + 알림 안 받음 선택 default를 알림 안 받음으로 선택 후 수정)
    ...
    try {
      const response = await $.ajax({
        ...
        data: {
          ...
          format: "json",
          errorformat: "json",
          key: mapKey, // 여기서 mapKey는 API 키
        },
      });

      if (response.response.result && response.response.result.items) {
        if (type === "start") {
          setStartSuggestions(response.response.result.items); // 여기를 수정
          setActiveInput("start");
        } else {
          setEndSuggestions(response.response.result.items); // 여기를 수정
          setActiveInput("end");
        }
      } else {
        if (type === "start") {
  ...
  const handleAddressChange = async (value, type) => {
    if (type === "start") {
      ...
      if (value.length > 2) {
        await searchAddress(value, "start");
      } else {
        setStartSuggestions([]);
      }
    } else {
  ...
  // 주소 선택 핸들러
  const selectAddress = (addressItem, type) => {
    if (type === "start") {
      setStartAddress(addressItem.title);
      sttPoint = addressItem.point;
    } else {
      setEndAddress(addressItem.title);
      dstPoint = addressItem.point;
    }
    setStartSuggestions([]);
    setEndSuggestions([]);
    setActiveInput(null);
  ...
  return (
    <>
      <div className="sidebar">
        ...
        <div className={`search-container ${showSearchBars ? "active" : ""}`}>
          {/* 출발지, 목적지 입력 필드와 검색 버튼 */}
            ...
            <div className="destination-search-container">
              ...
              <div className="autocomplete-container">
                {(activeInput === "start"
                  ...
                  <div
                    key={index}
                    onClick={() => selectAddress(item, activeInput)}
                  >
                ...
                <div className="nav-container">
                  <div className="search-container">
                    ...
                    <button
                      id="searchBtn"
                      onClick={() =>
                        getAddress(searchTerm, handleSearchResults)
                      }
                    >
                      검색
                    </button>
